...
public sealed class CodecRegistry : PhDisposable
{
    ...
    public ICodec? SelectMetadataCodec(string filePath)
    {
        lock (_lock)
        {
            foreach (var codec in _metadataCodecs)
            {
                try
                {
                    if (codec.CanLoadMetadata(filePath)) return codec;
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"❌❌❌ {nameof(SelectMetadataCodec)} ({codec.CodecId}): {ex.Message}");
                }
            }
        }
    ...
    public ICodec? SelectDecodeCodec(PhotoMetadata metadata, CodecSelectionContext context)
    {
        ...
        lock (_lock)
        {
            foreach (var codec in _decodeCodecs)
            {
                try
                {
                    if (codec.CanDecode(metadata, context)) return codec;
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"❌❌❌ {nameof(SelectDecodeCodec)} ({codec.CodecId}): {ex.Message}");
                }
            }
        }
