...
{
    private readonly NativePluginHandle _plugin;
    ...
    public NativeCodecProxy(
        NativePluginHandle plugin,
        IGCodecApi* codecApi,
        SDK.CodecPluginCapability capability,
        NativePluginQuarantine quarantine)
    {
        _plugin = plugin;
        _codecApi = codecApi;
    ...
    private PhotoMetadata LoadMetadataCore(string filePath, CancellationToken token)
    {
        ...
        var cancelHandle = NativeHostApiTable.RegisterCancellation(token);
        try
        {
        ...
        {
            NativeHostApiTable.ReleaseCancellation(cancelHandle);
        }
        return meta;
    }
